...
```{=html}
<!-- SS: Ok for me but very quickly into equations.  
See if I keep-->
```
...
random walk: the size of the step is choosen to be reasonable \* too small: the chain moves very slowly \* too big: we can get stuck
...
::: notes
Frequentist optimise the likelihood Bayesian combine it with prior
...
**Objective**: model the radial growth of individual tree, acounting for the effect of ontogeny on growth
...
get the species growth potential (median of indiv Gmax) =\> 0.34 (Eschweilera) to 4.5 (Cecropia) cm DBH/an

use traits to explain species growth potential with a linear model =\> effect of WD (-), fine root diameter (-), leaf calcium content (-), leaf N content (+) =\> consistent with fast acquisitive vs slow conservative
...
::::: columns
::: {.column width="50%"}
![](Niche.png){fig-align="center" width="60%"}  
...
-   $\color{royalblue}{\gamma}$ is the abundance of the tree species in the community (the intercept directly related to the mean probability)

-   $\color{mediumpurple}{\psi}$ is the effect of spatial autocorrelation of the species represented by a matrix of the $K$ Moran's eigenvectors ($V$) most explanatory of the species' presence
:::
:::::
